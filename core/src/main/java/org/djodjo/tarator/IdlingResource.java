 ...
 * default, {@link Tarator} synchronizes all view operations with the UI thread as well as
 * AsyncTasks; however, it has no way of doing so with "hand-made" resources. In such cases, test
 * authors can register the custom resource and {@link Tarator} will wait for the resource to
...
public interface IdlingResource {
  ...
  /**
   * Returns {@code true} if resource is currently idle. Tarator will <b>always</b> call this
   * method from the main thread, therefore it should be non-blocking and return immediately.
   */
  public boolean isIdleNow();

  /**
   * Registers the given {@link ResourceCallback} with the resource. Tarator will call this method:
   * <ul>
  ...
  /**
   * Registered by an {@link IdlingResource} to notify Tarator of a transition to idle.
   */
