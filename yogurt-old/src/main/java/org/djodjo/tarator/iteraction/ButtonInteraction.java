...
public final class ButtonInteraction extends TextViewInteraction {
  ...
  @Inject
  ButtonInteraction(UiController uiController, ViewFinder viewFinder, @MainThread Executor mainThreadExecutor, FailureHandler failureHandler, Matcher<View> viewMatcher, AtomicReference<Matcher<Root>> rootMatcherRef) {
  ...
  public TextViewAssert assertThat() {
    ...
    runSynchronouslyOnUiThread(new Runnable() {
      ...
      public void run() {
        ...
        tva[0] = new TextViewAssert((Button)targetView);
      }
    });
    return tva[0];
  }
